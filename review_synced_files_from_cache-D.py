...
def analyze_cache(cache):
    ...
    if df.empty:
        print("⚠️ Cache file is empty. Nothing to review.")
        return df, pd.DataFrame(columns=["Extension", "Count"])

    print("📋 Available columns in cache:", df.columns.tolist())
...
def main():
    ...
    if not df.empty:
        ...
        print("\n📄 Full File List: [No data available]")
